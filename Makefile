# This Makefile is for the Murmilad::Panda extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Murmilad/Panda.pm]
#     AUTHOR => [q[Murmilad <murmilad@gmail.com>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[Murmilad::Panda]
#     PM => { Murmilad/Panda.pm=>q[lib/Murmilad/Panda.pm], Murmilad/Panda_1.pm=>q[lib/Murmilad/Panda_1.pm] }
#     PREREQ_PM => {  }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Murmilad/Panda.pm]
...
MAKE = make
FULLEXT = Murmilad/Panda
BASEEXT = Panda
PARENT_NAME = Murmilad
DLBASE = $(BASEEXT)
VERSION_FROM = lib/Murmilad/Panda.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
INST_LIBDIR      = $(INST_LIB)/Murmilad
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Murmilad
...
DISTNAME = Murmilad-Panda
DISTVNAME = Murmilad-Panda-0.01
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3SECTION) --perm_rw=$(PERM_RW) -u \
	  Murmilad/Panda.pm $(INST_MAN3DIR)/Murmilad::Murmilad::Panda.$(MAN3EXT) \
	  Murmilad/Panda_1.pm $(INST_MAN3DIR)/Murmilad::Murmilad::Panda_1.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) '  - '\''Murmilad <murmilad@gmail.com>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Murmilad-Panda' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '      "Murmilad <murmilad@gmail.com>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Murmilad-Panda",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="Murmilad-Panda" VERSION="0.01">' > Murmilad-Panda.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl extension for blah blah blah</ABSTRACT>' >> Murmilad-Panda.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Murmilad &lt;murmilad@gmail.com&gt;</AUTHOR>' >> Murmilad-Panda.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> Murmilad-Panda.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-thread-multi-5.40" />' >> Murmilad-Panda.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> Murmilad-Panda.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> Murmilad-Panda.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> Murmilad-Panda.ppd
